...
a4 = exp(a4);
a4 = bsxfun(@rdivide, a4, sum(a4)); % Stores the hypotheses for each training example
...
% cost_graddecay = (lambda / 2) * (sum(sum(stack{1}.w .* stack{1}.w)) + sum(sum(stack{2}.w .* stack{2}.w)) + sum(sum(softmaxTheta .* softmaxTheta)));
cost_graddecay = (lambda / 2) * sum(sum(softmaxTheta .* softmaxTheta));
cost_softmax = (-1/M) * sum(sum(log(a4) .* groundTruth));
cost = cost_softmax + cost_graddecay;
